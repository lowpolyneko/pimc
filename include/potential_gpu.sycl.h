        ...
        int numPoints)
{
    ...
    stream.submit([&](sycl::handler& cgh) {
        ...
        sycl::local_accessor<double, 1> point(sycl::range<1>(7), cgh);
        sycl::local_accessor<int, 1> cutoff(sycl::range<1>(1), cgh);
        cgh.parallel_for(
            ...
            if (lid == 0) {
                ...
                cutoff[0] = (rho < 3.0 && z < 2.0) ? 1 : 0;
                point[0] = finx;
                point[1] = finy;
                point[2] = z;
                point[3] = rho;
                point[4] = finx / xscale0;
                point[5] = finy / xscale1;
                point[6] = (z - xoffset2) / xscale2;
            ...
            const double finx = point[0];
            const double finy = point[1];
            const double z = point[2];
            const double rho = point[3];
            const double rp0 = point[4];
            const double rp1 = point[5];
            const double rp2 = point[6];
            ...
            if (lid == 0) {
                ...
                if (rho > 5.0 || z > 6.5) {
                    ...
                    const double p5 = -normm3 * 10395.0 * sin6 * sycl::cos(6.0 * ph0) * x12 / r10;
